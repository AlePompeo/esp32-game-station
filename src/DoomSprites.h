#pragma once
#include <Arduino.h>

// ── Font 4×6 ─────────────────────────────────────────────────────────────────
#define DOOM_FONT_W      24   // bytes per row
#define DOOM_FONT_H      6
#define DOOM_CHAR_W      4
#define DOOM_CHAR_H      6
#define DOOM_CHAR_MAP    " 0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ.,-_(){}[]#"
static const uint8_t doom_bmp_font[] PROGMEM = {
  ,
};

// ── Logo 72×47 ────────────────────────────────────────────────────────────────
#define DOOM_LOGO_W  72
#define DOOM_LOGO_H  47
static const uint8_t doom_bmp_logo[] PROGMEM = {
  ,
};

// ── Gun 32×32 ─────────────────────────────────────────────────────────────────
#define DOOM_GUN_W   32
#define DOOM_GUN_H   32
static const uint8_t doom_bmp_gun[] PROGMEM = {
  ,
};
static const uint8_t doom_bmp_gun_mask[] PROGMEM = {
  ,
};

// ── Fire 24×20 ────────────────────────────────────────────────────────────────
#define DOOM_FIRE_W  24
#define DOOM_FIRE_H  20
static const uint8_t doom_bmp_fire[] PROGMEM = {
  0x00,0x00,0x00, 0x00,0x18,0x00, 0x00,0x0c,0x00, 0x02,0x77,0x00,
  0x01,0x67,0x00, 0x01,0xe5,0x80, 0x01,0xe3,0xc0, 0x02,0xa2,0xc0,
  0x07,0x82,0xe0, 0x1f,0x41,0xf9, 0x9d,0x80,0x7c, 0x3e,0x00,0x5e,
  0x76,0x00,0x6a, 0x38,0x00,0x36, 0x2f,0x80,0x5c, 0x36,0x00,0x7e,
  0x3f,0x00,0x58, 0x10,0x00,0x3c, 0x88,0x00,0x00, 0x00,0x00,0x00,
};

// ── Imp (enemy) 32×32, 5 frames ───────────────────────────────────────────────
#define DOOM_IMP_W    32
#define DOOM_IMP_H    32
#define DOOM_IMP_N    5
static const uint8_t doom_bmp_imp[] PROGMEM = {
  ,
  // frame 1
  ,
  // frame 2
  ,
  // frame 3 (hit)
  ,
  // frame 4 (dead)
  ,
};
static const uint8_t doom_bmp_imp_mask[] PROGMEM = {
  ,
  // frame 1 mask
  ,
  // frame 2 mask
  ,
  // frame 3 mask (hit)
  ,
  // frame 4 mask (dead)
  ,
};

// ── Fireball 16×16 ────────────────────────────────────────────────────────────
#define DOOM_FB_W   16
#define DOOM_FB_H   16
static const uint8_t doom_bmp_fireball[] PROGMEM = {
  0x00,0x00, 0x01,0x40, 0x0a,0xb0, 0x0e,0xd0, 0x00,0x68, 0x53,0xb4, 0x0f,0x48, 0x27,0x78,
  0x17,0xa8, 0x27,0xf0, 0x21,0xd6, 0x02,0xf8, 0x20,0x48, 0x06,0x20, 0x01,0x00, 0x00,0x00,
};
static const uint8_t doom_bmp_fireball_mask[] PROGMEM = {
  0x1f,0x40, 0x0f,0xf0, 0x3f,0xf8, 0x1f,0xfc, 0x7f,0xfd, 0x7f,0xfc, 0x7f,0xfd, 0xff,0xfe,
  0xff,0xff, 0xff,0xff, 0xff,0xfe, 0xff,0xfe, 0x3f,0xfe, 0x17,0xf8, 0x07,0xf4, 0x01,0xe0,
};

// ── Items 16×16, 2 frames ─────────────────────────────────────────────────────
#define DOOM_ITEM_W   16
#define DOOM_ITEM_H   16
#define DOOM_ITEM_N   2
static const uint8_t doom_bmp_items[] PROGMEM = {
  0x1f,0xf8, 0x3f,0xfc, 0x7f,0xfe, 0x7f,0xfe, 0x77,0xee, 0x3f,0xfc, 0x5f,0xfa, 0x2f,0xf6,
  0x53,0xcc, 0x3e,0x7e, 0x5e,0x7c, 0x38,0x1e, 0x58,0x1c, 0x3e,0x7e, 0x5e,0x7e, 0x2e,0xfc,
  0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x0f,0xfc, 0x17,0xfc, 0x22,0x6c,
  0x36,0x44, 0x3f,0xfc, 0x1f,0xfc, 0x2b,0xfc, 0x05,0x54, 0x02,0xa8, 0x00,0x00, 0x00,0x00,
};
static const uint8_t doom_bmp_items_mask[] PROGMEM = {
  0x1f,0xf8, 0x3f,0xfc, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe,
  0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x7f,0xfe, 0x3f,0xfc,
  0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x0f,0xfc, 0x1f,0xfc, 0x3f,0xfc,
  0x3f,0xfc, 0x3f,0xfc, 0x3f,0xfc, 0x3f,0xfc, 0x07,0xfc, 0x03,0xf8, 0x00,0x00, 0x00,0x00,
};

// ── Gradient shading table ────────────────────────────────────────────────────
// 8 levels, each a 16×8 dither pattern (2 bytes × 8 rows = 16 bytes)
#define DOOM_GRAD_W   2
#define DOOM_GRAD_H   8
#define DOOM_GRAD_N   8
static const uint8_t doom_gradient[] PROGMEM = {
  ,
};

// bit_mask for read_bit — MSB first
static const uint8_t PROGMEM doom_bit_mask[8] = { 128, 64, 32, 16, 8, 4, 2, 1 };
#define doom_read_bit(b, n)  ((b) & pgm_read_byte(doom_bit_mask + (n)) ? 1 : 0)
